...
/**
 * Server-side proxy to the Railway relay's /pet/evolve — same shape and
...
export async function POST(req: NextRequest) {
  ...
  if (!body || typeof body.player !== "string" || typeof body.stage !== "number") {
    return NextResponse.json({ error: "invalid body" }, { status: 400 });
  }

  try {
    const upstream = await fetch(`${relayUrl}/pet/evolve`, {
      method: "POST",
      headers: { "Content-Type": "application/json", "x-relay-secret": relaySecret },
      body: JSON.stringify({ player: body.player, stage: body.stage }),
    });
